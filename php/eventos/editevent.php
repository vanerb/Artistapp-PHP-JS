...
if (isset($nombre) && isset($ubicacion) && isset($link) && isset($fechafin) && isset($fechainicio)) {
    if (!empty($nombre) && !empty($fechainicio) && !empty($fechafin)) {
        if (strlen($nombre) > 0 && strlen(trim($nombre)) == 0) {
        } else {
            $inicio_fecha = date("Y/m/d", strtotime($fechainicio));
            $fin_fecha = date("Y/m/d", strtotime($fechafin));
            $imagenant = $_POST['imgant'];

            if ($inicio_fecha < $fin_fecha) {
                $database = new Crud();
                $sqlConnection = new Connection();
                if (0 < $_FILES['imgeventoedit']['error'][0]) {
                    echo 'Error: ' . $_FILES['imgeventoedit']['error'][0] . '<br>';
                } else {
                    ...
                    if($id_tipo == 3){
                        ...
                        if ($_FILES['imgeventoedit']['name'][0] != "") {
    
                            $result = $database->getinfo("SELECT * FROM eventos WHERE id='$id_evento'");
                            ...
                            $file_type = mime_content_type($_FILES['imgeventoedit']['tmp_name'][0]);
    
                            if (!in_array($file_type, $allowed_types)) {
                                echo "error3";
                            } else {
                                if (strlen($nombre) < 144 && strlen($ubicacion) < 80 && strlen($link) < 244) {
                                    $nombretrim = strtolower(trim($nombre));
                                    $result = $database->getinfo("SELECT COUNT(*) as total FROM eventos WHERE LOWER(TRIM(nombre)) = '$nombretrim' AND id <> '$id_evento'");
                                    $count = $result->fetch_assoc()["total"];
                                    if ($count == 0) {
                                        $max_file_size = 2 * 1024 * 1024; // 2MB en bytes
                                        if ($_FILES["imgeventoedit"]["size"][0] > $max_file_size) {
                                            echo "error7";
                                        }
                                        else{
                                            $nombre_archivo = $_FILES['imgeventoedit']['name'][0];
                                            $temp_archivo = $_FILES['imgeventoedit']['tmp_name'][0];
                                            ...
                                            $database->EditarEvento($data, $id_evento);
        
        
                                            if ($imagen != "media/ni-image.jpg") {
        
                                                $result = $database->getinfo("SELECT COUNT(*) as total FROM eventos WHERE imagen='$imagen'");
                            ...
                            if (strlen($nombre) < 144 && strlen($ubicacion) < 80 && strlen($link) < 244) {
                                ...
                                if ($count == 0) {
                                    ...
                                    $data = array($nombre, $ubicacion, $link, $inicio_fecha, $fin_fecha, $id, $imagenant);
                        ...
                        if ($_FILES['imgeventoedit']['name'][0] != "") {
                            ...
                            if (strlen($nombre) < 144 && strlen($ubicacion) < 80 && strlen($link) < 244) {
                                ...
                                if ($count == 0) {
                                    ...
                                    $data = array($nombre, $ubicacion, $link, $inicio_fecha, $fin_fecha, $usuario_select, $imagenant);
